...
| 版本 | v1.1 |
| 状态 | **已完成（M-01~M-08 全部落地，见附录 D；本文档作为体验层优化存档保留）** |
| 编制日期 | 2026-09-02 |
| 编制人 | 前端负责人（FE） |
...
| v1.1 | 2026-09-22 | — | 校订：状态改为已完成；术语表中 Tools 页承载项更新为现网 4 个标签（导入 / 新建 Agent 已随模板系统下线） |
...
| Tools（业务工具） | 页面化的跨 Agent 工具集：同步 / 超级同步 / 跨 Agent 编辑 / 对比（导入与新建 Agent 已随模板系统下线） |
| Data（数据中心） | 页面化的只读视角：统计面板 / 检查报告 / 审计日志 |
...
| UC-06 | Data 页层级与结果回跳 | 进入数据中心 | 逐个切换 3 个 tab；在检查报告中点击一条结果 | 无重复标题；点击结果跳回 Workbench 并定位到对应文件行 | M-02 |
...
| UC-06 | Data 页层级 | ✅ 3 个 tab（统计面板 / 检查报告 / 审计日志）；无 `.modal-header` |
...
## 附录 E：第二轮优化 —— 文档编辑体验（2026-09-02）

> 范围：主工作台的**文档编辑**链路。承接附录 D（M-01~M-08）之后的体验层二次收敛。

### E.1 优化清单

| 优先级 | 项 | 落地内容 |
|---|---|---|
| P0 | 默认视图模式改源码编辑 | 默认 `edit`（原来每次切文件强制回到 `preview`）；偏好持久化到 `soulforge.editor.view`；工具栏常驻提示「预览保存会规范化 Markdown 格式」 |
| P0 | 降低预览往返的格式损耗 | turndown 新增**任务列表**规则（保留 `- [ ]` / `- [x]`）与**表格列对齐**保留（读 `align` / `text-align`，回写 `:---:` / `---:`）；`DOMPurify` 增加 `ADD_ATTR: ['align']` |
| P0 | 「重新加载磁盘版本」 | 工具栏入口，放弃未保存修改并重读磁盘（有修改时二次确认）；同时清除该文档草稿 |
| P0 | 草稿保护 | 未保存内容防抖 1.5s 落 `localStorage`（`soulforge.drafts`，单条上限 200KB）；**按 key 增量写入**，避免多标签页互相清空；重开时询问「恢复 / 丢弃」 |
| P0 | 会话恢复 | `soulforge.session` 记录打开的窗口与左侧 Agent；重开后按需重读文件还原（最多 3 个窗口） |
| P1 | lint 行内标记 | 警告注入 Monaco Markers（波浪线 + 悬浮规则与建议 + 滚动条标记）；`F8` / `Shift+F8` 跳转 |
| P1 | 文档状态条 | 编辑器底部：`第 L 行, 第 C 列 · N 字符 · ≈T tokens`；选中显示已选字符数；token 估算（CJK≈1/字，其余 4 字符/token）超 2000 转警告色 |
| P1 | 文档删除入口 | 接入后端既有 `DELETE /api/agents/{id}/files/{path}`（走回收站）；未保存的文档拒绝删除 |
| P1 | `Ctrl+B` 让位给加粗 | 折叠左栏仅保留 `Alt+1`/`Alt+2`；编辑器聚焦时 `Ctrl+B` 加粗、`Ctrl+I` 斜体（Monaco `addCommand`） |
| P1 | 多窗口快捷键 | `Ctrl+1/2/3` 切换编辑窗口、`Ctrl+Shift+S` 全部保存、`Alt+W` 关闭当前（未用 `Ctrl+W`：浏览器保留键无法拦截） |
| P2 | Markdown 大纲 | 工具栏「大纲」或 `Ctrl+Shift+O`；解析 H1–H6 并跳过围栏代码块；点击跳转（预览模式先切回源码编辑） |
| P2 | 新建文档 | 复用 `PUT` 的创建能力写入标题骨架后直接打开 |
| P2 | 预览模式 `Ctrl+F` 桥接 | 预览是 contentEditable 无查找能力，按 `Ctrl+F` 自动切回源码编辑并打开查找框 |
| P2 | 编辑器工具栏收敛 | ~~「应用预设 / AI 整理 / 导出」收进「整理 ▾」下拉；主位保留 保存 / 重新加载 / 历史 / 检查~~（**0.5.0 已回退**：三个动作平铺为一级按钮，下拉层级取消；工具栏改为折行布局承接窄窗口） |
| P2 | 文件树过滤与键盘导航 | 过滤输入框（命中高亮、`Esc` 清空）、`↑`/`↓` 移动并打开、`Enter` 打开；容器可聚焦 |
| P2 | 仅显示有警告 | 文件树开关，持久化到 `soulforge.filetree.warnOnly` |
| P2 | 文件树折叠记忆 | 分组折叠状态持久化到 `soulforge.filetree.collapsed`（原来切 Agent 即重置） |
| P2 | 命令面板「最近打开」 | 空查询置顶最近打开的文件（`soulforge.recent.files`，上限 8，MRU 去重） |
| P2 | 回滚 diff 预览 | 接入后端既有 `GET /api/diff/history`（前端此前未暴露）；确认弹窗内嵌差异预览，加载中禁止确认 |

### E.2 实现要点与偏差

| 项 | 说明 |
|---|---|
| 快捷键冲突处理 | `Ctrl+B` 从「折叠左栏」改为编辑器「加粗」；折叠统一 `Alt+1`/`Alt+2`。关闭文档用 `Alt+W` 而非 `Ctrl+W`（浏览器保留键，`preventDefault` 无效） |
| 草稿写入策略 | 初版为「整体覆盖」，会清掉其他标签页的草稿；已改为**只增量写脏条目**，删除仅发生在保存成功 / 关闭窗口 / 重新加载 / 删除文档四个明确节点 |
| 引导版本 | `soulforge.intro-v2` → `intro-v3`，使存量用户重新看到更新后的快捷键说明（因 `Ctrl+B` 语义变更） |
| 未改动项 | 预览模式的 HTML 注释仍无法保留（`DOMPurify` 渲染阶段即剥离），已通过工具栏提示与「默认源码模式」规避 |

### E.3 验证结果

| 验证项 | 方式 | 结果 |
|---|---|---|
| 类型检查 + 构建 | `npm run build`（`tsc --noEmit && vite build`） | ✅ 通过（退出码 0） |
| 运行时界面验证 | 自动化只读走查 | ⏸ **未执行** —— 应用 localStorage 为同源共享，若在用户正在使用的会话中另开页面，会话恢复逻辑可能触发草稿「恢复/丢弃」询问并影响其未保存内容。已改为提供人工核对清单 |
| 后端回归 | `pytest` | 未涉及后端改动（仅消费既有接口），预期无回归 |

### E.4 人工核对清单（建议在刷新后逐项确认）

1. 打开文档 → 默认进入**源码编辑**（不再是预览）；切到预览后切换文件，模式保持不变
2. 工具栏：主位为 保存 / 重新加载 / 历史 / 检查，其余在「整理 ▾」（0.5.0 起「整理 ▾」已取消，应用预设 / AI 整理 / 导出当前文件 均为一级按钮）
3. 编辑内容后 `Ctrl+S` → 路径栏出现「已保存 HH:mm:ss」；点「重新加载」可放弃修改回磁盘版本
4. 编辑后**不保存**直接刷新页面 → 询问「发现草稿是否恢复」（选恢复应回到未保存内容）
5. 开 2–3 个文档 → `Ctrl+1/2/3` 切换激活窗口；`Ctrl+Shift+S` 全部保存；`Alt+W` 关闭当前
6. 光标放到正文 → `Ctrl+B` 加粗选中文本（不再折叠左栏）；`Alt+1`/`Alt+2` 折叠左右栏
7. 点「检查」→ 有警告时正文出现行内波浪线，`F8` 依次跳转
8. 编辑器底部状态条显示行列 / 字符数 / ≈tokens
9. 点「大纲」或 `Ctrl+Shift+O` → 列出标题并点击跳转；预览模式下按 `Ctrl+F` 应自动切到源码编辑并打开查找
10. 文件树：输入过滤词应立即筛选并高亮命中；`↑`/`↓` 移动并打开；「仅警告」可筛选存在警告的文件
11. 文件树「＋ 新建」创建文档、「删除」把文档移入回收站（先确认提示文案）
12. 历史 → 「回滚版本」→ 弹窗内应显示与当前内容的差异，加载完成前确认按钮不可点
13. `Ctrl+K` → 空查询时顶部出现「最近打开」分组
14. 重启浏览器再打开应用 → 上次打开的窗口被还原

---

## 附录 F：对比工具专项修复 —— 「相同内容被误判为不同」（2026-09-02）

> 范围：业务工具页的「对比」与「同步」两条链路；涉及后端 `diff_service` / `sync_service` / `diff` API 与前端 `DiffModal` / `SyncModal` / `HistoryModal`。

### F.1 根因定位（真实数据证据）

对真实工作区（7 Agent × 6 个 CORE 文件 = 30 组跨 Agent 对比）做全量扫描，**发现 5 组「视觉完全一致却被判为不同」**，全部为 `SOUL.md`：

| 观测项 | 修复前 |
|---|---|
| 判定结果 | `相似度 0.9991`，`unified_diff` 非空（有差异） |
| 首处差异位置 | 字符偏移 1625/1628（**文件末尾**） |
| 差异内容 | 右侧多出 `\n\n\u200d`（空行 + **U+200D 零宽连接符**） |
| 行数 | 左 71 行 / 右 73 行 |

**根因（三条）**

1. **主因：文件末尾残留不可见字符**（零宽连接符 U+200D + 多余空行）。零宽字符在编辑器中完全不可见，`\u200d` 通常是模型生成内容时的残留；旧实现直接对原始字符串做 `difflib` 比较，于是被判为差异。
2. **缺少任何归一化**：BOM、CRLF/CR、全角空格、不换行空格、Tab、行尾空白、多余空行、文末换行——旧实现一概视为差异。
3. **相似度在原始文本上计算**：即便差异纯属噪声，也会得到 0.9991 这类「接近但不等于 1」的数值，UI 显示 99%，用户合理感知为「被判为不同」。

### F.2 修复方案

**后端 `diff_service.py` 重构**（见 `backend/app/services/diff_service.py`）

| 机制 | 说明 |
|---|---|
| 两档归一化口径 | `ignore_whitespace`（默认）：忽略 BOM / 换行风格 / 零宽字符 / 伪空格 / 连续空白与缩进 / 行尾空白 / 多余空行 / 首尾空行；`strict`：只忽略 BOM / 换行风格 / 零宽字符 |
| 双口径分离 | **比较**用 `canonical()`（激进归一，判定「有效业务内容」）；**展示**用 `display_text()`（保留空行与缩进，只清不可见噪声）——避免为了判定一致而让差异视图丢失行结构、不可读 |
| 两级快路径 | ① 字节级相同 → 直接返回一致；② 归一化后相同 → 返回「有效内容一致」并跳过全部 diff 计算 |
| 相似度口径对齐 | 在归一化文本上计算，**内容一致 ⇒ 相似度恒为 1.0**，不再出现 99% 的误导 |
| 噪声归因 | `noise_kinds`（8 类）**只在差异可被噪声完整解释时**才输出；存在真实差异时返回空，避免把无关噪声误报为原因 |
| 单次计算 | `unified_diff` 只算一次，HTML 高亮复用其结果（旧实现 `html_diff` 内部会再算一遍 diff） |
| 空差异兜底 | 一致时 `unified_diff` / `html_diff` 为空字符串，前端据此显示明确结论而非空框 |

**API**：`GET /api/diff` 与 `GET /api/diff/history` 新增可选 `mode` 参数（非法值回落默认）；响应新增 `identical` / `noise_kinds` / `mode`；`POST /api/sync/plan` 的每个文件新增 `identical` / `noise_kinds`。

**前端**：新增 `components/DiffView.tsx` 统一差异视图（一致时显示结论 + 被忽略的噪声类型）；`DiffModal` 增加口径开关、切换口径自动重算；`SyncModal` 标注「✓ 内容一致」并汇总提示；`HistoryModal` 回滚预览复用同一视图。

### F.3 性能优化与压测结果

**优化点**：① 一致内容走快路径，不做 diff；② 消除重复的 diff 计算；③ 长文本自动降级为**行级**相似度（阈值 20000 字符），规避字符级 `SequenceMatcher` 的 O(n²)；④ `autojunk=False` 避免长文档相似度失真；⑤ HTML 高亮改为单遍拼接。

**真实大文件压测**（取工作区最大的 12 个 `.md`，最大 172KB）：

| 场景 | 修复前 | 修复后 | 提升 |
|---|---|---|---|
| 12 个真实大文件合计 | 64386.4 ms | 1328.1 ms | **48.5×** |
| 单文件最高（DREAMS.md 172KB） | 26655.8 ms | 148.1 ms | **179.9×** |
| 内容完全一致（约 300KB / 8000 行） | 3234.8 ms | ≈ 0 ms | 快路径（不做 diff） |
| 仅格式噪声（约 300KB） | 3232.8 ms | 240.3 ms | 约 13×，且判定为一致 |

**测试内预算**（`test_diff_normalization.py` 断言）：一致快路径 < 50ms；噪声快路径 < 500ms；大文本真实差异 < 2s。

### F.4 测试验证体系

新增 `backend/tests/test_diff_normalization.py`（**38 项**）：

| 类别 | 覆盖内容 |
|---|---|
| 完全相同 | 字节级一致 → `identical=True`、相似度 1.0、无差异内容 |
| 仅格式噪声（13 种变体） | CRLF / CR / 文末无换行 / 行尾空白 / 全角空格 / NBSP / 零宽空格 / 零宽连接符（行内与文件末尾）/ Tab 缩进 / 多余空行 / BOM / BOM+CRLF → 全部判为「有效内容一致」 |
| 真实差异（6 种变体） | 改词 / 增行 / 删行 / 改标题 / 行序变化 / 追加句子 → 全部仍判为不一致，且不能被归一化吞掉 |
| 复合场景 | 真实差异 + 格式噪声同时存在 → 仍判为不一致，且 `noise_kinds` 为空 |
| 严格口径 | 5 类「绝对无意义噪声」在 strict 下忽略；6 类空白/空行差异在 strict 下保留 |
| 噪声归因 | 8 类噪声分别可被正确归因；真实差异不产生噪声归因 |
| 安全性 | 差异视图对 HTML 做转义（`<script>` → `&lt;script&gt;`） |
| 准确率 | 噪声变体 + 真实差异共 19 个用例，识别准确率 **19/19 = 100%** |
| 性能压测 | 快路径、大文本真实差异、相似度降级模式（`lines` / `chars`） |
| API 集成 | `identical` 字段、`mode` 参数、非法 `mode` 回落、同步计划 `identical` 标记 |

### F.5 回归结果

| 验证项 | 结果 |
|---|---|
| 后端全量测试 | **168 tests / 0 failed / 0 errors / 1 skipped**（基线 129 passed / 1 skipped，新增 38 项） |
| 真实数据端到端复验（经 API） | 原 5 组误判 → 现 `identical=True`、`similarity=1.0`、噪声归因 `invisible_char, trailing_whitespace, multiple_spaces, blank_lines, edge_blank_lines`；25 组真实差异仍全部正确识别 |
| 前端类型检查 + 构建 | `tsc --noEmit` 与 `vite build` 通过 |
| 兼容性 | `unified_diff` / `similarity` / `html_diff` 旧签名保留，AI 整理 / 预设 / 同步等调用方不受影响 |

### F.6 顺带修复的其他缺陷

| 缺陷 | 说明 |
|---|---|
| 默认 A/B 选到同一 Agent | `initialAgent` 为 `null` 时 `agents.find(a => a.id !== null)` 恒返回 `agents[0]`，与 Agent A 相同 → 默认变成「自己和自己比」；已改为选取第一个不同 Agent，并在 A/B 相同时自动纠正 |
| 相同内容时差异区为空白 | 旧实现返回 `<pre class='diff-view'></pre>`，用户看到空框易误判为故障；现显示明确结论 + 噪声说明 |
| 上一轮引入的 CSS 类名不匹配 | `.rollback-diff` 下写的是 `diff-added` / `diff-removed`，而实际渲染类为 `diff-add` / `diff-del` → 回滚差异预览高亮不生效；已修正 |
